  ...
  return (
    <div className="flex flex-col items-start gap-4">
      <button
        ...
        className={`${isVisible ? "border-transparent text-blue-500/80" : "border-b-neutral-700 text-neutral-400"} flex w-full cursor-pointer justify-between border-b-2  pb-2 text-4xl font-medium md:text-5xl`}
      >
        ...
        <>
          ...
          <ul className="flex flex-wrap justify-center gap-x-5 gap-y-3 text-xs">
            {tech.map((item, index) => (
              <li
                key={index}
                className="rounded-xl border border-neutral-700 px-3 py-2 font-medium text-neutral-500"
              >
                {item}
              </li>
            ))}
